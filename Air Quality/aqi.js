document.getElementById("submit").addEventListener('click', function (e) {
    ...
    airReportElement.innerHTML = 'City: '+city;
    var options = {
		method: 'GET',
		headers: {
    ...
    .then(function(response){
      if(response && response.error){
        airReportElement.innerHTML='<p>An Error Occured' +response.error.message+ '</p>';
      }
      else{
    ...
    .catch(function(err){
        console.error(err);
        airReportElement.innerHTML='<p> An Unknown Error Occured,Please Try Later';
    });
